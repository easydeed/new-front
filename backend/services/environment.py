...
MANIFEST: Tuple[EnvVar, ...] = (
    ...
    EnvVar("NOTIFICATIONS_ENABLED", OPTIONAL,
           ...
           "accident."),
    EnvVar("STRIPE_WEBHOOK_SECRET", REQUIRED,
           "THE ENTIRE PAID PATH. Stripe signs every event with it; "
           "without it signature verification fails, the handler returns "
           "400, Stripe retries and gives up — and a customer who has "
           "been charged stays on the free plan with nothing anywhere "
           "saying so.\n"
           "\n"
           "RECLASSIFIED FROM OPTIONAL (MONEY1). The old entry read: "
           "'OPTIONAL here ONLY because the webhook path already refuses "
           "unverified payloads rather than trusting them — the failure "
           "is closed, not silent.' That is right about SECURITY and "
           "wrong about AVAILABILITY. Closed-failure means a forged "
           "payload cannot fake an upgrade; it equally means every "
           "LEGITIMATE event is refused, identically and indistinguishably "
           "from outside.\n"
           "\n"
           "So the classification defeated the mechanism built to catch "
           "exactly this: the boot report names missing REQUIRED "
           "variables, and this one told it not to look. It was also "
           "absent from render.yaml, so neither place a human reads "
           "mentioned it.\n"
           "\n"
           "THE INVERSION IS WORTH KEEPING. ALLOWED_ORIGINS sat here "
           "marked REQUIRED and was read by nothing — a claim this file "
           "could not support, and the owner acted on it. This sat marked "
           "OPTIONAL and the paid path depends on it. Both are the same "
           "error: a classification asserted rather than checked.\n"
           "\n"
           "RESIDUAL EXPOSURE, DESIGNED: with STRICT_ENV=1 and this "
           "unset, the API refuses to boot. That is correct — a service "
           "whose paid path cannot work should not pretend otherwise — "
           "and STRICT_ENV is deliberately unset until the owner has "
           "confirmed production carries every required variable."),
