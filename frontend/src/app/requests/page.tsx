...
function SharedDeedsTracker() {
  ...
  /**
   * WHICH ROW THE LINK WAS ABOUT — decided in `lib/requestsFocus`, not
   * here. A review is a `deed_shares.id` and a signing is a
   * `signing_requests.id`, so on one page `?focus=42` names two
   * different rows; `?kind=` carries what the old paths carried in the
   * path itself, and an ambiguous link highlights nothing rather than
   * guessing. That last rule is a negative one and cannot be shown by
   * reading this file, so it lives where a test can call it.
   */
                      ...
                      return (
                        <tr
                          ...
                          className={`border-b border-slate-100 hover:bg-slate-50 transition-colors ${
                            isFocused(deed.id, "reviews", focus)
