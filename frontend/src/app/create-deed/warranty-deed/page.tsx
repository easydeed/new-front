// Minimal test wizard for Warranty Deed (CA) (safe canary route)
'use client';
...
export default function Page() {
  const [data, setData] = useState<any>({ 
    ...
    grantors_text: '',
    grantees_text: '',
    execution_date: new Date().toISOString().slice(0,10), covenants:''
  });
  ...
  async function generate(){
    try {
      ...
      const a = document.createElement('a'); a.href=url; a.download='Warranty_Deed_CA_'+Date.now()+'.pdf'; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
    } catch(e:any) { setErr(e.message); } finally { setBusy(false); }
  }

  return (<main style={{maxWidth:800, margin:'0 auto', padding:'1rem'}}>
    <h1>Warranty Deed (CA)</h1>
    ...
    <label>Covenants (optional)</label>
    <input value={data.covenants} onChange={e=>setField('covenants', e.target.value)} style={{width:'100%',marginBottom:8}}/>
