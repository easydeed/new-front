// Minimal test wizard for Quitclaim Deed (CA) (safe canary route)
'use client';
...
export default function Page() {
  ...
  async function generate(){
    try {
      ...
      const res = await fetch('/api/generate/quitclaim-deed-ca', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(data) });
      if(!res.ok) throw new Error((await res.json()).detail || 'Failed');
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a'); a.href=url; a.download='Quitclaim_Deed_CA_'+Date.now()+'.pdf'; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
    } catch(e:any) { setErr(e.message); } finally { setBusy(false); }
  }

  return (<main style={{maxWidth:800, margin:'0 auto', padding:'1rem'}}>
    <h1>Quitclaim Deed (CA)</h1>
