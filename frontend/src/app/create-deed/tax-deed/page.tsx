// Minimal test wizard for Tax Deed (CA) (safe canary route)
'use client';
...
export default function Page() {
  const [data, setData] = useState<any>({ 
    ...
    grantors_text: '',
    grantees_text: '',
    execution_date: new Date().toISOString().slice(0,10), tax_sale_ref:''
  });
  ...
  async function generate(){
    try {
      ...
      const res = await fetch('/api/generate/tax-deed-ca', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(data) });
      if(!res.ok) throw new Error((await res.json()).detail || 'Failed');
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a'); a.href=url; a.download='Tax_Deed_CA_'+Date.now()+'.pdf'; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  ...
  return (<main style={{maxWidth:800, margin:'0 auto', padding:'1rem'}}>
    ...
    <label>Tax Sale Reference</label>
    <input value={data.tax_sale_ref} onChange={e=>setField('tax_sale_ref', e.target.value)} style={{width:'100%',marginBottom:8}}/>
