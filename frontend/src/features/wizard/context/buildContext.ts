 ...
 * ✅ PHASE 19c: Enhanced requested_by field with robust fallbacks
 */
export function toBaseContext(s: WizardStore) {
  ...
  return {
    // ✅ PHASE 19c: Enhanced requested_by with multiple fallbacks
    requested_by: step2?.requestedBy || 
                  s.formData?.requestedBy || 
                  s.formData?.requested_by ||
                  step2?.titleCompany || 
                  '',
