...
**Provenance note.** The investigation was run against `34de330`, which
carried Doctrine A but not Doctrine B — B was built and awaiting CI at
the time. Both have since merged, so §6's "what exists after A and B"
describes shipped state rather than anticipated state. Nothing else in
this report was affected: the partner API, the key lifecycle, the
webhook surface and the deep-link path are untouched by either.
