...
const base = path.join(__dirname, 'rescuepatch-6');
...
`);
  ...
  code = code.replace(/const\s+onNext\s*=\s*useCallback\(async\s*\(\s*\)\s*=>\s*\{([\s\S]*?)\n\s*\},\s*\[[^\]]*\]\);/, (m, body) => {
...
`);
  ...
  code = code.replace(/\}\s*else\s*\{([\s\S]*?)(console\.error\([^;]+;\s*alert\([^\)]+\);?\s*\}[\s\S]*?)\}(?=\s*\}(?:,\s*\[[^\]]+\]\);|;\s*(?:\/\/|const|let|var|return|function|export|$)))/g, (m, inner, errorBlock) => {
