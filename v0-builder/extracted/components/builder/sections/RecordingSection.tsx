...
export function RecordingSection({ requestedBy, returnTo, onChange }: RecordingSectionProps) {
  ...
  useEffect(() => {
    if (!requestedBy && partners.length > 0) {
      const lastUsed = typeof window !== "undefined" ? localStorage.getItem("lastPartnerUsed") : null
      if (lastUsed) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Recording Requested By</label>
        <div className="relative">
          ...
          <select
            ...
            className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary appearance-none bg-white"
          >
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">When Recorded, Return To</label>
