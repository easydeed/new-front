...
export function TransferTaxSection({ value, onChange, city, deedType }: TransferTaxSectionProps) {
  ...
  }, [city, deedType, onChange, value])

  const calculatedAmount = useMemo(() => {
    if (!value || value.isExempt || !value.transferValue) return ""

    const amount = Number.parseFloat(value.transferValue.replace(/[^0-9.]/g, ""))
    if (isNaN(amount)) return ""

    const countyTax = (amount / 1000) * 1.1
    const cityTax = value.areaType === "city" ? (amount / 1000) * 4.5 : 0
  ...
  }, [calculatedAmount, onChange, value])
        ...
        <div className="space-y-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  const formatted = raw ? Number.parseInt(raw).toLocaleString() : ""
          ...
          {calculatedAmount && (
            <div className="flex items-center gap-2 p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
              ...
              <span className="font-medium text-emerald-700">Documentary Transfer Tax: ${calculatedAmount}</span>
            </div>
          )}
        </div>
      )}
