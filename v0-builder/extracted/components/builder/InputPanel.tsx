...
import { InputSection, type SectionStatus } from "./InputSection"
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
            ...
            {Object.values(statuses).map((status, i) => (
              <div
                ...
                className={`w-2.5 h-2.5 rounded-full transition-colors ${
                  status === "complete" ? "bg-emerald-500" : status === "warning" ? "bg-amber-500" : "bg-gray-300"
                }`}
              />
            ))}
          </div>
      ...
      <div className="p-4 bg-white border-t border-gray-200 flex-shrink-0">
        ...
        {!isReady && <p className="text-center text-sm text-gray-500 mt-2">Complete all sections to generate</p>}
      </div>
    </div>
  )
}
