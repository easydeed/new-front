...
| `document_id` | No | `"DOC-2026-A7X9K"` | QR verification code |
| `qr_code_data` | No | `"data:image/png;base64,..."` | QR code image |
...
8. **QR Verification** — Scannable verification (see QR Verification System below)
...
### Overview
Every deed generated by DeedPro includes a scannable QR code linking to a verification page. This allows anyone to confirm the document was authentically generated by DeedPro.

### Components

| Component | Location | Purpose |
|-----------|----------|---------|
| Database Tables | `backend/migrations/004_document_authenticity.sql` | `document_authenticity` + `verification_log` |
| Short Code Generator | `backend/utils/short_code.py` | Generates `DOC-YYYY-XXXXX` codes |
| QR Generator | `backend/utils/qr_generator.py` | Creates base64 QR code images |
| Verification API | `backend/routers/verification.py` | Public `/api/verify/{code}` endpoint |
| Verification Page | `frontend/src/app/verify/[code]/page.tsx` | Displays verification results |

### Document ID Format
```
DOC-2026-A7X9K
└─┬─┘ └┬─┘ └─┬─┘
  │    │    └── 5 random chars (no ambiguous: 0,O,I,L,1)
  │    └─────── Year
  └──────────── Prefix
```
...
### Verification Flow
```
1. User scans QR code on deed
2. Redirected to: deedpro-frontend-new.vercel.app/verify/DOC-2026-XXXXX
3. Frontend calls: GET /api/verify/{code}?method=qr_scan
4. Backend looks up document_authenticity table
5. Returns: valid/revoked/not_found + document metadata
...
- id (UUID PK)
- short_code (unique, e.g., "DOC-2026-A7X9K")
- document_type ("grant_deed", "quitclaim_deed", etc.)
- property_address, property_apn, county
- grantor_display, grantee_display (abbreviated names)
- content_hash (SHA-256)
- status ("active", "revoked", "superseded")
- verification_count, first_verified_at, last_verified_at

-- verification_log (audit trail)
- document_id → document_authenticity
- verification_method ("qr_scan", "manual", "api")
...
interface VerificationResponse {
  ...
  message: string;
  document?: {
    id: string;           // "DOC-2026-A7X9K"
    type: string;         // "Grant Deed"
    propertyAddress: string;
    apn: string;
    county: string;
    grantor: string;      // Abbreviated: "JOHN S."
    grantee: string;      // Abbreviated: "JANE D."
    generatedAt: string;  // ISO date
    contentHash: string;  // Abbreviated hash
...
```python
from routers.verification import create_document_authenticity

# During deed generation, create authenticity record
...
template_vars["document_id"] = auth_data["short_code"]
template_vars["qr_code_data"] = auth_data["qr_code_data"]
```
